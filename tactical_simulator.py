"""
Tactical Simulator for UAV DDoS Defense and CPU Management

This environment simulates a single UAV making decisions about:
...
class TacticalUAVEnv(gym.Env):
    ...
    def _decode_action(self, action: int):
        """Decode flat action into (model_idx, freq_idx, cores_idx)."""
        model_idx = action // 12
        remaining = action % 12
        freq_idx = remaining // 3
        cores_idx = remaining % 3
    ...
    def _calculate_reward(self, model_idx, freq_idx, deescalate: bool):
        ...
        if deescalate:
            ...
            # Encourage when battery critical and threat low, penalize under high threat
